...
Not surprising as haven't defined what the action does yet. 6. Back in `click_outside.ts`, TS needs a workaround to get the type definition correct:
...
7. Now, when add paramaters to click*outside: `export const click_outside: Action = function (node) {`, TS correctly assigns the type to node: *(parameter) node: Node extends HTMLElement\_
8.
