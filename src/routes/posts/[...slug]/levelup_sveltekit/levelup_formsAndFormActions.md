...
export const actions = {
	default: async ({ locals, request }) => {
		...
		if (!locals?.user?.roles?.includes('admin')) {
			// if user roles does not include admin as a role, fail
			return fail(401, {
...
```javascript
<script>import {enhance} from "$app/forms"; export let form; $: console.log('form', form);</script>
```
...
```javascript
<form use:enhance action="?/testAction" method="POST">
	// form
</form>
```
